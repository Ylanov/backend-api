...
import { loadYMaps } from "../lib/ymaps"; // Предполагаем, что этот файл существует из старых компонентов
...
export default function DashboardMap({ zones, tasks, center = [55.751244, 37.618423], zoom = 9 }: Props) {
  ...
  useEffect(() => {
    ...
    loadYMaps(KEY).then((ymaps) => {
      ...
      zones.forEach(zone => {
        const polygonCoords = zone.points.map(p => [p.lat, p.lng]);
      ...
      tasks.forEach(task => {
        if (!task.zone) return; // Не рисуем задачи без зоны
        const zoneCoords = task.zone.points.map(p => [p.lat, p.lng] as [number, number]);
        const taskCenter = getPolygonCenter(zoneCoords);

        const placemark = new ymaps.Placemark(taskCenter, {
          balloonContentHeader: `Задача: ${task.title}`,
          balloonContentBody: `
            <strong>Статус:</strong> ${task.status}<br>
            <strong>Приоритет:</strong> ${task.priority}<br>
            <strong>Команда:</strong> ${task.team?.name || 'Не назначена'}
          `,
  ...
  }, [zones, tasks, center, zoom]); // Перерисовываем при изменении данных

  return (
    <div
      ref={containerRef}
      style={{ width: "100%", height: "100%", minHeight: '75vh', borderRadius: 8 }}
    />
  );
}