  ...
  return (
    <div className='bg-white/95 backdrop-blur-md nice-shadow rounded-lg w-[700px] overflow-hidden flex flex-col -space-y-2'>
      <div className='flex flex-col -space-y-2 px-4 pt-4'>
        <div className='text-gray-500 px-3 py-0.5 rounded-full font-semibold text-xs'>Курс </div>
        <div className='text-black px-3 py-0.5 rounded-full text-lg font-bold'>Добавить новое обновление курса</div>
      </div>
      <div className='px-5 -py-2'>
        <FormLayout onSubmit={formik.handleSubmit}>
          ...
          <div className='flex justify-end py-2'>
            <button onClick={() => setSelectedView('list')} className='text-gray-500 px-4 py-2 rounded-md text-sm font-bold antialiased'>Отмена</button>
            <button className='bg-black  text-white px-4 py-2 rounded-md text-sm font-bold antialiased'>Добавить обновление</button>
          </div>
        </FormLayout>
      </div>
    </div>
        ...
        <div key={update.id} className='py-2 border-b border-neutral-200 antialiased'>
          <div className='font-bold text-gray-500 flex space-x-2 items-center justify-between '>
            <div className='flex space-x-2 items-center'>
              ...
              <span
                title={"Создано " + dayjs(update.creation_date).format('D MMMM YYYY')}
