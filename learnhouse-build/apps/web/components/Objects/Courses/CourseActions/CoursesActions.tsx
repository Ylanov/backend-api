...
function CoursesActions({ courseuuid, orgslug, course, trailData }: CourseActionsProps) {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      ...
      try {
        ...
        toast.error('Не удалось проверить доступ к курсу. Пожалуйста, попробуйте позже.')
        setHasAccess(false)
      }
    }
  ...
  const handleCourseAction = async () => {
    ...
    const loadingToast = toast.loading(
      isStarted ? 'Выход из курса...' : 'Начало курса...'
    )
    ...
    try {
      if (isStarted) {
        ...
        toast.success('Вы успешно покинули курс', { id: loadingToast })
      } else {
        await startCourse('course_' + courseuuid, orgslug, session.data?.tokens?.access_token)
        mutate(`${getAPIUrl()}trail/org/${org?.id}/trail`)
        toast.success('Вы успешно начали курс', { id: loadingToast })
      ...
      toast.error(
        isStarted
          ? 'Не удалось покинуть курс. Пожалуйста, попробуйте позже.'
          : 'Не удалось начать курс. Пожалуйста, попробуйте позже.',
        { id: loadingToast }
      )
    } finally {
  ...
  const handleApplyToContribute = async () => {
    ...
    const loadingToast = toast.loading('Отправка заявки на участие...')

    try {
      const data = {
        message: "Я хотел бы стать соавтором этого курса."
      }

      await applyForContributor('course_' + courseuuid, data, session.data?.tokens?.access_token)
      await revalidateTags(['courses'], orgslug)
      await refetch()
      toast.success('Ваша заявка на участие успешно отправлена', { id: loadingToast })
    } catch (error) {
      console.error('Failed to apply as contributor:', error)
      toast.error('Не удалось отправить заявку. Пожалуйста, попробуйте позже.', { id: loadingToast })
    } finally {
    ...
    if (!session.data?.user) {
      return (
        <>
          ...
          <span>{action === 'start' ? 'Начать курс' : 'Покинуть курс'}</span>
  ...
  const renderContributorButton = () => {
    ...
    if (!session.data?.user) {
      return (
        <button
          ...
          aria-label="Зарегистрируйтесь, чтобы подать заявку на участие в курсе"
    ...
    if (contributorStatus === 'PENDING') {
      return (
        <div className="w-full bg-amber-50 text-amber-700 border border-amber-200 py-3 rounded-lg nice-shadow font-semibold flex items-center justify-center gap-2 mt-3">
          ...
          Заявка на участие рассматривается
        </div>
      );
    }
    ...
    return (
      <button
        ...
        aria-label="Подать заявку на участие в курсе"
            ...
            <div className="flex items-center gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="text-sm font-medium text-gray-900">Готовы начать?</div>
                    <div className="text-sm text-gray-500">
                      Начните обучение с {totalActivities} увлекательных {totalActivities === 1 ? 'задания' : 'заданий'}
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1">
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  aria-label={`Посмотреть прогресс курса: ${completedActivities} из ${totalActivities} заданий выполнено`}
                  className="flex-1 text-left hover:bg-neutral-50/50 p-2 rounded-lg transition-colors"
                >
                  <div className="text-sm font-medium text-gray-900">Прогресс курса</div>
                  <div className="text-sm text-gray-500">
                    {completedActivities} из {totalActivities} заданий выполнено
                  </div>
                </button>
              </div>
            </div>
            ...
            <>
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg nice-shadow">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-green-800 font-semibold">Вам доступен этот курс</h3>
                </div>
                <p className="text-green-700 text-sm mt-1">
                  Вы приобрели этот курс и имеете полный доступ ко всему контенту.
                </p>
              </div>
              <button
                onClick={handleCourseAction}
                disabled={isActionLoading}
                aria-label={isStarted ? 'Покинуть этот курс' : 'Начать этот курс'}
            ...
            <>
              <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg nice-shadow">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-amber-800 font-semibold">Платный курс</h3>
                </div>
                <p className="text-amber-700 text-sm mt-1">
                  Этот курс требует покупки для доступа к контенту.
                </p>
              </div>
              <Modal
                ...
                dialogTitle="Купить курс"
                dialogDescription="Выберите способ оплаты для доступа к курсу"
                minWidth="sm"
              />
              <button
                ...
                aria-label="Купить курс для получения доступа"
              >
