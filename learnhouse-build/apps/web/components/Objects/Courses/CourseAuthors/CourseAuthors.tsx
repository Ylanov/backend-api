...
const MultipleAuthors = ({ authors, isMobile }: { authors: Author[], isMobile: boolean }) => {
  ...
  return (
    <div className="flex flex-col items-center space-y-4 px-2 py-2">
      <div className="text-[12px] text-neutral-400 font-semibold self-start">Авторы и обновления</div>
            ...
            <>
              {displayedNames.map((author, index) => (
                <span key={author.user.user_uuid}>
                  ...
                  {index === 0 && authors.length > 1 && index < displayedNames.length - 1 && " и "}
                </span>
              ))}
              {authors.length > 2 && (
                <span className="text-neutral-500 ml-1">
                  и еще {authors.length - 2}
                </span>
              )}
            </>
          )}
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center space-x-3">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-semibold text-neutral-600">Обновления курса</span>
          </div>
          {updates && updates.length > 0 && (
            <span className="px-2 py-0.5 text-[11px] font-medium bg-neutral-100 text-neutral-500 rounded-full">
              {updates.length} {updates.length === 1 ? 'обновление' : 'обновлений'}
            </span>
          )}
        </div>
        {adminStatus.isAdmin && (
          ...
          >
            <PencilLine size={12} />
            <span>{selectedView === 'new' ? 'Отмена' : 'Новое обновление'}</span>
          </button>
        )}
      </div>
...
const NewUpdateForm = ({ setSelectedView }: { setSelectedView: (view: string) => void }) => {
  ...
  const formik = useFormik({
    ...
    validate: (values) => {
      const errors: any = {}
      if (!values.title) errors.title = 'Заголовок обязателен'
      if (!values.content) errors.content = 'Содержание обязательно'
      return errors
    },
    onSubmit: async (values) => {
      ...
      if (res.status === 200) {
        toast.success('Обновление успешно добавлено')
        ...
        <FormField name="title">
          ...
          <Form.Control asChild>
            <Input
              ...
              type="text"
              required
              placeholder="Что нового в этом обновлении?"
        ...
        <FormField name="content">
          ...
          <Form.Control asChild>
            <Textarea
              ...
              required
              placeholder="Поделитесь подробностями вашего обновления..."
  ...
  if (!updates || updates.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-8 px-4 text-center bg-neutral-50/50 rounded-lg border border-dashed border-neutral-200">
        ...
        <p className="text-sm text-neutral-600 font-medium">Нет обновлений</p>
        <p className="text-xs text-neutral-400 mt-1">Обновления об этом курсе появятся здесь</p>
      </div>
    )
  }
          ...
          <div className="flex items-start justify-between">
            <div className="space-y-1 min-w-0 flex-1">
              <div className="flex items-baseline space-x-2">
                ...
                <span
                  title={dayjs(update.creation_date).format('D MMMM YYYY')}
...
const DeleteUpdateButton = ({ update }: any) => {
  ...
  const handleDelete = async () => {
    const toast_loading = toast.loading('Удаление обновления...')
    ...
    if (res.status === 200) {
      ...
      toast.error('Не удалось удалить обновление')
    }
  }

  return (
    <ConfirmationModal
      confirmationButtonText="Удалить обновление"
      confirmationMessage="Вы уверены, что хотите удалить это обновление?"
      dialogTitle="Удалить обновление?"
