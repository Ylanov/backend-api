...
function RolesUpdate(props: Props) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    const toastId = toast.loading("Обновление роли...")
    if (res.status === 200) {
      await mutate(`${getAPIUrl()}orgs/${org.id}/users`)
      props.setRolesModal(false)
      toast.success("Роль обновлена", {id:toastId})
    } else {
      setIsSubmitting(false)
      setError('Ошибка ' + res.status + ': ' + res.data.detail)
      toast.error("Ошибка при обновлении роли", {id:toastId})
    }
  }
        ...
        <FormField name="course-visibility">
          ...
          >
            ...
            <FormMessage match="valueMissing">
              Пожалуйста, выберите роль для пользователя
            </FormMessage>
          </Flex>
