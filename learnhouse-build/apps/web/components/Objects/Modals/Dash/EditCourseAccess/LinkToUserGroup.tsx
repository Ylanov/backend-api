...
function LinkToUserGroup(props: LinkToUserGroupProps) {
    ...
    const handleLink = async () => {
        ...
        if (res.status === 200) {
            ...
            toast.success('Курс успешно привязан к группе пользователей')
    ...
    return (
        <div className='flex flex-col space-y-1 '>
            <div className='flex bg-yellow-100 text-yellow-900 mx-auto w-fit mt-3 px-4 py-2 space-x-2 text-sm rounded-full items-center'>
                <Info size={19} />
                <h1 className=' font-medium'>Пользователи, не входящие в эту UserGroup, больше не будут иметь доступа к курсу</h1>
            </div>
            <div className='p-4 flex-row flex justify-between items-center'>
                {usergroups?.length >= 1 &&
                    <div className='py-1'>
                        <span className='px-3 text-gray-400 font-bold rounded-full py-1 bg-gray-100 mx-3'>Название UserGroup</span>
                ...
                <div className='py-3'>
                    <button onClick={() => { handleLink() }} className='bg-green-700 text-white font-bold px-4 py-2 rounded-md shadow-sm'>Привязать</button>
                </div>
            </div>
        </div>
