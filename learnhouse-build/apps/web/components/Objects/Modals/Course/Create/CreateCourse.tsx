...
const validationSchema = Yup.object().shape({
  ...
  description: Yup.string()
    .max(1000, 'Должно быть не более 1000 символов'),
...
function CreateCourseModal({ closeModal, orgslug }: any) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      const toast_loading = toast.loading('Создание курса...')
  ...
  const handleUnsplashSelect = async (imageUrl: string) => {
    ...
    try {
      ...
      toast.error('Не удалось загрузить изображение с Unsplash')
    }
    ...
    <FormLayout onSubmit={formik.handleSubmit} >
        ...
        <div className="w-auto bg-gray-50 rounded-xl outline outline-1 outline-gray-200 h-[200px] shadow-sm">
          <div className="flex flex-col justify-center items-center h-full">
			...
			<FormField name="learnings">
				<FormLabelAndMessage
					label="Чему вы научите? (Результаты обучения)"
      ...
      <FormField name="visibility">
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="true">Публичный (Доступен для просмотра в интернете)</SelectItem>
            <SelectItem value="false">Приватный (Доступен только пользователям)</SelectItem>
          </SelectContent>
        </Select>
      </FormField>
