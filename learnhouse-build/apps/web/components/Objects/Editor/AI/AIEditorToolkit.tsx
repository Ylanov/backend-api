...
const UserFeedbackModal = (props: AIEditorToolkitProps) => {
  ...
  const getPrompt = (args: AIPromptsLabels) => {
    ...
    if (label === 'Writer') {
      return `Напиши 3 предложения про ${selection}`
    } else if (label === 'ContinueWriting') {
      return `Продолжи писать еще 3 предложения на основе "${selection}"`
    } else if (label === 'MakeLonger') {
      return `Сделай этот текст длиннее : "${selection}"`
    } else if (label === 'GenerateQuiz') {
...
const AiEditorToolButton = (props: any) => {
  ...
  // Перевод лейблов кнопок
  const getButtonLabel = (label: string) => {
    switch (label) {
      case 'Writer': return 'Написать';
      case 'ContinueWriting': return 'Продолжить';
      case 'MakeLonger': return 'Расширить';
      case 'GenerateQuiz': return 'Тест';
  ...
  return (
    ...
    >
      ...
      <span>{getButtonLabel(props.label)}</span>
    </button>
  )
}
    ...
    <div>
        ...
        !aiEditorState.error.isError && (
          <div className="flex flex-col mx-auto justify-center align-middle items-center">
            <p className="mx-auto flex p-2 text-white/80 mt-4 font-bold justify-center text-sm align-middle">
              Поместите курсор в конец предложения, чтобы продолжить писать{' '}
            </p>
            <div
              onClick={() => {
                handleOperation(
      ...
      {aiEditorState.selectedTool === 'MakeLonger' &&
        ...
        !aiEditorState.error.isError && (
          <div className="flex flex-col mx-auto justify-center align-middle items-center">
            <p className="mx-auto flex p-2 text-white/80 mt-4 font-bold justify-center text-sm align-middle">
              Выделите текст, чтобы сделать его длиннее{' '}
            </p>
            <div
              onClick={() => {
                handleOperation(
        ...
        !aiEditorState.error.isError && (
          <div className="flex flex-col mx-auto justify-center align-middle items-center">
            <div className="mx-auto flex p-2 text-white/80 mt-4 font-bold justify-center text-sm align-middle space-x-6">
              <p>Перевести выделенный текст на </p>
              <input
                value={aiEditorState.chatInputValue}
                onChange={handleChange}
                placeholder="Японский, Арабский, Немецкий и т.д. "
      ...
      {aiEditorState.isWaitingForResponse && !aiEditorState.error.isError && (
        <div className="flex flex-col mx-auto justify-center align-middle items-center">
          ...
          <p className="font-bold mt-4 text-white/90">Думаю...</p>
        </div>
      )}
