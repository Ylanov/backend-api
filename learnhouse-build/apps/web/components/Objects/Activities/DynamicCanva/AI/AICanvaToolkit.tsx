...
function AIActionButton(props: {
  ...
  const getPrompt = (label: string, selection: string) => {
    if (label === 'Объяснить') {
      return `Объясни эту часть курса "${selection}", учитывая контекст этого курса.`
    } else if (label === 'Обобщить') {
      return `Обобщи это "${selection}", учитывая контекст курса.`
    } else if (label === 'Перевести') {
      return `Переведи "${selection}" на другой язык.`
    } else {
      return `Приведи примеры, чтобы лучше понять "${selection}", по возможности дай контекст в рамках курса.`
    }
  }
  ...
  const tooltipLabel =
    props.label === 'Объяснить'
      ? 'Объяснить слово или предложение с помощью ИИ'
      : props.label === 'Обобщить'
        ? 'Обобщить длинный абзац или текст с помощью ИИ'
        : props.label === 'Перевести'
          ? 'Перевести на другие языки с помощью ИИ'
          : 'Привести примеры для лучшего понимания с помощью ИИ'
  return (
    <div className="flex space-x-2">
      <ToolTip sideOffset={10} slateBlack content={tooltipLabel}>
        ...
        >
          {props.label === 'Объяснить' && <BookOpen size={16} />}
          {props.label === 'Обобщить' && <FormInput size={16} />}
          {props.label === 'Перевести' && <Languages size={16} />}
          {props.label === 'Примеры' && (
            <div className="text-white/50">Пр</div>
          )}
          <div>{props.label}</div>
        </button>
      </ToolTip>
