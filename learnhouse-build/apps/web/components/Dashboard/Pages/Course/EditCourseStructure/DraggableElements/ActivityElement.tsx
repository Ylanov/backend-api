...
function ActivityElement(props: ActivitiyElementProps) {
  ...
  async function deleteActivityUI() {
    const toast_loading = toast.loading('Удаление занятия...')
    // Assignments
  ...
  async function changePublicStatus() {
    const toast_loading = toast.loading('Обновление задания...')
  ...
  async function updateActivityName(activityId: string) {
    if (
      ...
      try {
        ...
        mutate(`${getAPIUrl()}courses/${props.course_uuid}/meta?with_unpublished_activities=${withUnpublishedActivities}`)
        await revalidateTags(['courses'], props.orgslug)
        toast.success('Название занятия успешно обновлено')
        router.refresh()
      } catch (error) {
        toast.error('Не удалось обновить название занятия')
        ...
        >
          ...
          <div className="flex items-center gap-2 justify-end">
            ...
            >
              ...
              <span>{!props.activity.published ? 'Опубликовать' : 'Скрыть'}</span>
            </button>
            <div className="w-px h-3 bg-gray-300 mx-1 self-center rounded-full hidden sm:block" />
            <ToolTip content="Предпросмотр занятия" sideOffset={8}>
              <Link
                href={
            ...
            {/*   Delete Button  */}
            <ConfirmationModal
              confirmationMessage="Вы уверены, что хотите удалить это занятие?"
              confirmationButtonText="Удалить занятие"
              dialogTitle={'Удалить ' + props.activity.name + ' ?'}
              dialogTrigger={
                <button
    ...
    <>
        ...
        <>
          ...
          >
            <div className="text-sky-100 font-bold text-xs flex items-center space-x-1">
              <FilePenLine size={12} />  <span>Редактировать страницу</span>
            </div>
          </Link>
        </>
      )}
      {activity.activity_type === 'TYPE_ASSIGNMENT' && (
        <>
          ...
          >
            <div className="text-sky-100 font-bold text-xs flex items-center space-x-1">
              <FilePenLine size={12} /> {!isMobile && <span>Редактировать задание</span>}
            </div>
          </Link>
        </>
      )}
