...
function ThumbnailUpdate({ thumbnailType }: ThumbnailUpdateProps) {
  ...
  const validateFile = (file: File, type: 'image' | 'video'): boolean => {
    if (type === 'image') {
      if (!VALID_IMAGE_MIME_TYPES.includes(file.type as ValidImageMimeType)) {
        showError(`Недопустимый тип файла: ${file.type}. Пожалуйста, загружайте только изображения PNG или JPG/JPEG`);
        return false;
      }

      if (file.size > MAX_FILE_SIZE) {
        showError(`Размер файла (${(file.size / 1024 / 1024).toFixed(2)} МБ) превышает лимит в 8 МБ`);
        return false;
      }
    } else {
      if (!VALID_VIDEO_MIME_TYPES.includes(file.type as ValidVideoMimeType)) {
        showError(`Недопустимый тип файла: ${file.type}. Пожалуйста, загружайте только видео MP4 или WebM`);
        return false;
      }

      if (file.size > MAX_VIDEO_FILE_SIZE) {
        showError(`Размер файла (${(file.size / 1024 / 1024).toFixed(2)} МБ) превышает лимит в 100 МБ`);
        return false;
      }
    }
  ...
  const handleUnsplashSelect = async (imageUrl: string) => {
    try {
      ...
      if (!VALID_IMAGE_MIME_TYPES.includes(blob.type as ValidImageMimeType)) {
        throw new Error('Неверный формат изображения от Unsplash');
      }
      ...
      showError('Не удалось обработать изображение Unsplash');
      setIsLoading(false);
    }
  }
  ...
  const updateThumbnail = async (file: File, type: 'image' | 'video') => {
    ...
    try {
      ...
      showError('Не удалось обновить обложку');
    } finally {
      setIsLoading(false);
    }
  }
      ...
      <div className="p-6">
        <div className="space-y-6">
          ...
          <p className="text-sm text-gray-500">
            {activeTab === 'image' && 'Поддерживаемые форматы: PNG, JPG/JPEG (макс. 8 МБ)'}
            {activeTab === 'video' && 'Поддерживаемые форматы: MP4, WebM (макс. 100 МБ)'}
          </p>
        </div>
      </div>
