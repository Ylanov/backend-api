...
function EditCourseContributors(props: EditCourseContributorsProps) {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                if (result.successful.length > 0) {
                    toast.success(`Успешно добавлено ${result.successful.length} соавтор(а/ов)`);
                }

                // Show error messages for failed adds
                result.failed.forEach(failure => {
                    toast.error(`Не удалось добавить ${failure.username}: ${failure.reason}`);
                });
            ...
            toast.error('Не удалось добавить соавторов');
        }
    };
    ...
    const updateContributor = async (contributorId: string, data: { authorship?: ContributorRole; authorship_status?: ContributorStatus }) => {
        try {
            ...
            if (currentContributor.authorship === 'CREATOR') {
                toast.error('Невозможно изменить роль или статус создателя');
                return;
            }
            ...
            if (res.status === 200 && res.data?.status === 'success') {
                toast.success(res.data.detail || 'Данные соавтора успешно обновлены');
                mutate(`${getAPIUrl()}courses/${courseStructure.course_uuid}/contributors`);
            } else {
                toast.error(`Ошибка: ${res.data?.detail || 'Не удалось обновить данные соавтора'}`);
            }
        } catch (error) {
            toast.error('Произошла ошибка при обновлении данных соавтора.');
        }
    };
    ...
    const handleBulkRemove = async () => {
        ...
        try {
            ...
            if (response.status === 200) {
                toast.success(`Успешно удалено ${selectedContributors.length} соавтор(а/ов)`);
                // Refresh contributors list
                mutate(`${getAPIUrl()}courses/${courseStructure.course_uuid}/contributors`);
                // Clear selection
            ...
            toast.error('Не удалось удалить соавторов');
        }
    };
                ...
                <div>
                    ...
                    <div className="mx-4 sm:mx-10 bg-white rounded-xl shadow-xs px-4 py-4">
                        <div className="flex flex-col bg-gray-50 -space-y-1 px-3 sm:px-5 py-3 rounded-md mb-3">
                            <h1 className="font-bold text-lg sm:text-xl text-gray-800">Соавторы курса</h1>
                            <h2 className="text-gray-500 text-xs sm:text-sm">
                                Управляйте соавторами и добавляйте новых в свой курс
                            </h2>
                        </div>
                        <div className="flex flex-col sm:flex-row sm:space-x-2 space-y-2 sm:space-y-0 mx-auto mb-3">
                            <ConfirmationModal
                                confirmationButtonText="Открыть для соавторов"
                                confirmationMessage="Вы уверены, что хотите открыть этот курс для соавторов?"
                                dialogTitle="Открыть для соавторов?"
                                dialogTrigger={
                                    <div className="w-full h-[200px] bg-slate-100 rounded-lg cursor-pointer hover:bg-slate-200 transition-all">
                                        ...
                                        <div className="flex flex-col space-y-1 justify-center items-center h-full p-2 sm:p-4">
                                            ...
                                            <div className="text-xl sm:text-2xl text-slate-700 font-bold">
                                                Открыть для соавторов
                                            </div>
                                            <div className="text-gray-400 text-sm sm:text-md tracking-tight w-full sm:w-[500px] leading-5 text-center">
                                                Курс открыт для соавторов. Пользователи могут подавать заявки на участие в улучшении контента курса.
                                            </div>
                                        </div>
                                    </div>
                                }
                            ...
                            />
                            <ConfirmationModal
                                confirmationButtonText="Закрыть для соавторов"
                                confirmationMessage="Вы уверены, что хотите закрыть этот курс для соавторов?"
                                dialogTitle="Закрыть для соавторов?"
                                dialogTrigger={
                                    <div className="w-full h-[200px] bg-slate-100 rounded-lg cursor-pointer hover:bg-slate-200 transition-all">
                                        {!isOpenToContributors && (
                                            <div className="bg-green-200 text-green-600 font-bold w-fit my-3 mx-3 absolute text-sm px-3 py-1 rounded-lg">
                                        ...
                                        <div className="flex flex-col space-y-1 justify-center items-center h-full p-2 sm:p-4">
                                            ...
                                            <div className="text-xl sm:text-2xl text-slate-700 font-bold">
                                                Закрыт для соавторов
                                            </div>
                                            <div className="text-gray-400 text-sm sm:text-md tracking-tight w-full sm:w-[500px] leading-5 text-center">
                                                Курс закрыт для соавторов. Только существующие соавторы могут изменять контент курса.
                                            </div>
                                        </div>
                                    </div>
                                }
                        ...
                        <div className="space-y-4">
                            <div className="relative">
                                ...
                                <Input
                                    placeholder="Поиск пользователей по имени или никнейму для добавления в соавторы..."
                                    value={searchQuery}
                            ...
                            <div className="bg-white rounded-xl nice-shadow">
                                {selectedContributors.length > 0 && (
                                    <div className="p-3 bg-gray-100 rounded-t-xl border-b">
                                        <div className="flex items-center justify-between">
                                            <span className="text-sm text-gray-700">
                                                Выбрано соавторов: {selectedContributors.length}
                                            </span>
                                            <div className="flex gap-2">
                                                <Button
