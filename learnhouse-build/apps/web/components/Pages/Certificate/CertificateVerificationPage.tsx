...
const CertificateVerificationPage: React.FC<CertificateVerificationPageProps> = ({ certificateUuid }) => {
  ...
  useEffect(() => {
    const fetchCertificate = async () => {
      try {
        ...
        setError('Не удалось проверить сертификат. Пожалуйста, попробуйте позже.');
        setVerificationStatus('invalid');
      } finally {
        setIsLoading(false);
      }
  ...
  const getVerificationStatusText = () => {
    switch (verificationStatus) {
      case 'valid':
        return 'Сертификат подтверждён';
      case 'invalid':
        return 'Сертификат не найден';
      case 'loading':
        return 'Проверка сертификата...';
      default:
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        <div className="bg-white rounded-2xl p-8 nice-shadow max-w-4xl w-full space-y-6">
          ...
          <div className="text-center">
            <h1 className="text-2xl font-bold text-gray-900 mb-2">Проверка сертификата</h1>
            <p className="text-gray-600">Пожалуйста, подождите, идёт проверка сертификата...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="bg-white rounded-2xl p-8 nice-shadow max-w-2xl w-full space-y-6">
          <div className="flex flex-col items-center space-y-4">
            ...
            <p className="text-gray-600 text-center">
              Сертификат с ID <span className="font-mono bg-gray-100 px-2 py-1 rounded">{certificateUuid}</span> не найден в нашей системе.
            </p>
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 w-full">
              ...
              <ul className="text-red-700 text-sm mt-2 list-disc list-inside space-y-1">
                <li>Указан неверный идентификатор сертификата</li>
                <li>Сертификат был отозван</li>
                <li>Срок действия сертификата истёк</li>
                <li>Сертификат был выдан другой организацией</li>
              </ul>
            </div>
        ...
        <div className="bg-white rounded-2xl p-6 mb-8 nice-shadow">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Проверка сертификата</h1>
                <p className="text-gray-600">Проверьте подлинность этого сертификата</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 nice-shadow">
              <h2 className="text-xl font-semibold text-gray-900 mb-4">Превью сертификата</h2>
              <div className="max-w-2xl mx-auto" id="certificate-preview">
                <CertificatePreview
                  ...
                  awardedDate={new Date(certificateData.certificate_user.created_at).toLocaleDateString('ru-RU', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                  })}
                  ...
                  <div className="w-20 h-12 bg-gray-100 rounded-lg overflow-hidden ring-1 ring-inset ring-black/10">
                    {certificateData.course.thumbnail_image ? (
                      <img
                        ...
                        alt={`Обложка курса ${certificateData.course.name}`}
                        ...
                        <div className="flex items-center space-x-1">
                          ...
                          {certificateData.course.authors.filter((author: any) => author.authorship_status === 'ACTIVE').length > 2 && (
                            <span className="text-neutral-400">
                              +{certificateData.course.authors.filter((author: any) => author.authorship_status === 'ACTIVE').length - 2} ещё
                            </span>
                          )}
                        </div>
                      </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl p-6 nice-shadow">
              <h2 className="text-xl font-semibold text-gray-900 mb-4">Информация о сертификате</h2>

              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">ID сертификата</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Тип сертификации</label>
                  <div className="bg-gray-50 p-3 rounded-lg">
                ...
                <div>
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <span className="text-gray-900">
                      {new Date(certificateData.certificate_user.created_at).toLocaleDateString('ru-RU', {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric',
                        hour: '2-digit',
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <h3 className="text-lg font-semibold text-blue-800">Информация о безопасности</h3>
              </div>
              <ul className="text-blue-700 text-sm space-y-2">
                <li>• Сертификат проверен по нашей защищённой базе данных</li>
                <li>• QR-код содержит ссылку для проверки подлинности</li>
                <li>• ID сертификата является криптографически устойчивым</li>
                <li>• Отметка времени проверена и подтверждена</li>
              </ul>
            </div>
          </div>
        </div>
