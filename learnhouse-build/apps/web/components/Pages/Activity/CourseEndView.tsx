  ...
  useEffect(() => {
    const fetchUserCertificate = async () => {
      ...
      if (!session?.data?.tokens?.access_token) {
        setCertificateError('Для просмотра сертификата требуется авторизация');
        return;
      }
      ...
      try {
        ...
        if (result.success && result.data && result.data.length > 0) {
          ...
          setCertificateError('Сертификат для этого курса не найден');
        }
      } catch (error) {
        console.error('Error fetching user certificate:', error);
        setCertificateError('Не удалось загрузить сертификат. Пожалуйста, попробуйте позже.');
      } finally {
    ...
    try {
      ...
      certificateDiv.innerHTML = `
        ...
        <div style="
          ...
          line-height: 1.5;
          max-width: 500px;
        ">${userCertificate.certification.config.certification_description || 'Настоящим подтверждается успешное прохождение курса.'}</div>
        ...
        ">
          ...
          <span>${userCertificate.certification.config.certification_type === 'completion' ? 'Завершение курса' :
            userCertificate.certification.config.certification_type === 'achievement' ? 'За достижения' :
            userCertificate.certification.config.certification_type === 'assessment' ? 'На основе оценки' :
            userCertificate.certification.config.certification_type === 'participation' ? 'За участие' :
            userCertificate.certification.config.certification_type === 'mastery' ? 'Освоение навыков' :
            userCertificate.certification.config.certification_type === 'professional' ? 'Профессиональное развитие' :
            userCertificate.certification.config.certification_type === 'continuing' ? 'Повышение квалификации' :
            userCertificate.certification.config.certification_type === 'workshop' ? 'Участие в семинаре' :
            userCertificate.certification.config.certification_type === 'specialization' ? 'Специализация' : 'Завершение курса'}</span>
        </div>
        ...
        ">
          <div style="margin: 8px 0; font-size: 14px; color: #374151;">
            <strong style="color: ${theme.primary};">ID сертификата:</strong> ${certificateId}
          </div>
          <div style="margin: 8px 0; font-size: 14px; color: #374151;">
            <strong style="color: ${theme.primary};">Выдан:</strong> ${new Date(userCertificate.certificate_user.created_at).toLocaleDateString('ru-RU', {
              year: 'numeric',
              month: 'long',
              day: 'numeric'
            })}
          ...
          ${userCertificate.certification.config.certificate_instructor ? 
            `<div style="margin: 8px 0; font-size: 14px; color: #374151;">
              <strong style="color: ${theme.primary};">Инструктор:</strong> ${userCertificate.certification.config.certificate_instructor}
            </div>` : ''
          }
        </div>
        ...
        <div style="
          ...
          color: #6b7280;
        ">
          Этот сертификат можно проверить по адресу ${qrCodeLink}
        </div>
      `;
      ...
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-4 relative overflow-hidden">
        ...
        <div className="bg-white rounded-2xl p-8 nice-shadow max-w-4xl w-full space-y-6 relative z-10">
          ...
          <p className="text-gray-500">
            Ваше усердие и труд окупились. Вы освоили весь материал этого курса.
          </p>
          ...
          {isLoadingCertificate ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-3 text-gray-600">Загрузка сертификата...</span>
            </div>
          ) : certificateError ? (
            ...
            <div className="space-y-4">
              <h2 className="text-2xl font-semibold text-gray-900">Ваш сертификат</h2>
            ...
            <div className="bg-gray-50 rounded-lg p-6">
              <p className="text-gray-600">
                Для этого курса нет доступных сертификатов. Свяжитесь с инструктором для получения дополнительной информации.
              </p>
            </div>
          )}
    ...
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-4">
        <div className="bg-white rounded-2xl p-8 nice-shadow max-w-2xl w-full space-y-6">
          ...
          <p className="text-xl text-gray-600">
            Вы делаете большие успехи в курсе
            <span className="font-semibold text-gray-900"> {courseName}</span>
          </p>
          ...
          {progressInfo && (
            <div className="bg-gray-50 rounded-lg p-6 space-y-4">
              <div className="flex items-center justify-center space-x-2">
                ...
                <span className="text-lg font-semibold text-gray-700">Прогресс курса</span>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="text-sm text-gray-500">
                  {progressInfo.completed} из {progressInfo.total} занятий завершено
                </div>
              </div>
            </div>
          )}
          ...
          <p className="text-gray-500">
            Отличная работа! Завершите оставшиеся задания, чтобы получить сертификат о прохождении курса.
          </p>
