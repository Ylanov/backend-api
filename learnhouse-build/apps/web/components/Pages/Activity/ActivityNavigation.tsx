      ...
      {isFloating ? (
        <>
          ...
          <button
            ...
            title={prevActivity ? `Предыдущая: ${prevActivity.name}` : 'Нет предыдущего шага'}
          >
            <ChevronLeft size={20} className="text-gray-800 shrink-0" />
            <div className="flex flex-col items-start">
              <span className="text-xs text-gray-500">Предыдущая</span>
              <span className="text-sm capitalize font-semibold text-left">
                {prevActivity ? prevActivity.name : 'Нет предыдущего шага'}
              </span>
            </div>
          </button>
          ...
          <button
            ...
            title={nextActivity ? `Следующая: ${nextActivity.name}` : 'Нет следующего шага'}
          >
            <div className="flex flex-col items-end">
              <span className="text-xs text-gray-500">Следующая</span>
              <span className="text-sm capitalize font-semibold text-right">
                {nextActivity ? nextActivity.name : 'Нет следующего шага'}
              </span>
            </div>
        ...
        <>
          ...
          <div className="text-sm text-gray-500 justify-self-center">
            {currentIndex + 1} из {allActivities.length}
          </div>
