...
export function OrgProvider({ children, orgslug }: { children: React.ReactNode; orgslug: string }) {
  ...
  if (orgError || orgsError) {
    return <ErrorUI message="Произошла ошибка при получении данных" />
  }
  ...
  if (!isOrgActive) {
    return <ErrorUI message="Эта организация больше не активна" />
  }
      ...
      <div className="flex flex-col py-10 mx-auto antialiased items-center space-y-6 bg-linear-to-b from-yellow-100 to-yellow-100/5 ">
        <div className="flex flex-row items-center space-x-5 rounded-xl ">
          ...
          <div className="flex flex-col">
            <p className="text-3xl font-bold text-yellow-700">
              Вы ещё не являетесь участником этой организации
            </p>
          </div>
        </div>
        <div className="flex space-x-4">
          ...
          >
            ...
            <span className="text-md font-bold">Главная</span>
          </a>
          <button
          ...
          >
            ...
            <span className="text-md font-bold">Выйти</span>
          </button>
        </div>
      </div>
    )
